...
# --- SYNCED: UNIVERSAL LLRD OPTIMIZER LOGIC ---
def get_optimizer_grouped_parameters(model, base_lr, weight_decay=0.01):
    """Assigns decaying learning rates to model layers. Supports DistilBERT and XLM-R."""
    decay_factor = 0.95
...
def evaluate_onnx_model(source_model_path, tokenizer_name, test_df, seed):
    """Dynamically exports, quantizes, and evaluates an ONNX model."""
    temp_onnx_dir = f"./temp_onnx_seed_{seed}"
    
    model = ORTModelForSequenceClassification.from_pretrained(source_model_path, export=True, provider="CPUExecutionProvider")
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
    ...
    if os.path.exists(unquantized_path): os.remove(unquantized_path)
    if os.path.exists(quantized_path): os.rename(quantized_path, unquantized_path)
...
def main():
    ...
    for config in configs:
        ...
        for seed in seeds:
            ...
            # Extrapolate Model D if processing Model B
